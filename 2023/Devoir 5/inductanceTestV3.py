...
def inductanceMegneticField(X,Z,Rsource,Zsource,Isource,data):
  ...
  Bx  = zeros(shape(X))
  Bz  = zeros(shape(X))
  for k in range(len(Rsource)):
    for j in range(len(X)):
        x = X[j]
        z = Z[j]
        for i in range(data.nTheta):
            theta = (2*i+1)*pi/data.nTheta
            Xi = Rsource[k] * cos(theta)
            Yi = Rsource[k] * sin(theta)
            Ri = ((x-Xi)**2+(-Yi)**2+(z-Zsource[k])**2)**(1/2)
            dyi = 2*pi*Rsource[k]*cos(theta)/data.nTheta
            dxi = -2*pi*Rsource[k]*sin(theta)/data.nTheta
            Bx[j] += sum(((z-Zsource[k])/(Ri)**3)*dyi)
